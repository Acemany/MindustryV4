...
public class MobileInput extends InputHandler implements GestureListener{
    ...
    private final float edgePan = ucore.scene.ui.layout.Unit.dp.scl(60f);
    ...
    public boolean zoom(float initialDistance, float distance){
        ...
        if(Math.abs(distance - initialDistance) > ucore.scene.ui.layout.Unit.dp.scl(100f) && !zoomed){
            int amount = (distance > initialDistance ? 1 : -1);
            renderer.scaleCamera(Math.round(ucore.scene.ui.layout.Unit.dp.scl(amount)));
            zoomed = true;
            return true;
        }
