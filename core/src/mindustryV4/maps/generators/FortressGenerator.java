...
public class FortressGenerator{
    ...
    void gen(){
        gen.setBlock(enemyX, enemyY, Blocks.coreShard, team);
        ...
        Array<IntPositionConsumer> passes = Array.with(
            ...
            (x, y) -> {
                ...
                if(block instanceof PowerTurret){
                    ...
                    Item[] type = turret.getAmmoTypes();
                    int index = ammoPerType.get(block.id, 0);
                    block.handleStack(type[index], block.acceptStack(type[index], 1000, tile, null), tile, null);
    ...
    Array<Block> find(Predicate<Block> pred){
        ...
        for(Block block : content.blocks()){
            if(pred.test(block) && block.isVisible()){
                out.add(block);
            }
        }
        return out;
